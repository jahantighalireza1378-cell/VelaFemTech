...
export default function CustomizePage() {
  ...
  // اضافه شدن nightCount به استیت پدها
  const [selections, setSelections] = useState({
    pads: { count: 0, brand: '', nightCount: 0 }, // nightCount اضافه شد
  ...
  useEffect(() => {
    ...
    if (savedCart) {
      ...
      // محاسبه پیش‌فرض شبانه (مثلاً یک سوم تعداد کل)
  ...
  const handleCountChange = (item: string, change: number) => {
    setSelections(prev => {
      ...
      // اگر آیتم پد بود و تعداد کل کم شد، باید چک کنیم تعداد شبانه بیشتر از کل نشود
      if (item === 'pads') {
         if (newItem.nightCount > newCount) {
             newItem.nightCount = newCount;
         }
      }
  ...
  const handleNightChange = (change: number) => {
    setSelections(prev => {
        ...
        // محدودیت‌ها: زیر صفر نیاید و از کل تعداد بیشتر نشود
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            {/* ردیف مخصوص پد با قابلیت تفکیک روز/شب */}
            <div className="border-b border-gray-50 last:border-0 pb-4 mb-4">
                 ...
                 {/* کنترلر تفکیک روز و شب */}
                 <div className="mt-3 bg-gray-50/80 rounded-xl p-3 flex flex-col sm:flex-row items-center justify-between gap-3 text-sm">
                    <div className="flex items-center gap-2 text-vela-navy">
                        <Sun size={16} className="text-orange-400" />
                        <span>{text.labels.day}: <strong>{selections.pads.count - selections.pads.nightCount}</strong></span>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2 text-vela-navy">
                            ...
                            <span>{text.labels.night}:</span>
                        </div>
                        <div className="flex items-center bg-white rounded-lg border border-gray-200">
                            <button onClick={() => handleNightChange(-1)} className="w-6 h-6 flex items-center justify-center hover:bg-gray-100"><Minus size={12}/></button>
                            <span className="w-6 text-center font-bold">{selections.pads.nightCount}</span>
                            <button onClick={() => handleNightChange(1)} className="w-6 h-6 flex items-center justify-center hover:bg-gray-100"><Plus size={12}/></button>
                        </div>
                    </div>
                 </div>
            </div>
...
console.log("✅ ویژگی تفکیک روز/شب (Day/Night Split) به پدها اضافه شد.");