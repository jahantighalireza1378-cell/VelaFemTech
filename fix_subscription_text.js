...
export default function BuilderPage() {
  ...
  const t: any = {
    FA: {
      ...
      // ✅ متن‌های دکمه‌ها که باید نمایش داده شوند
            ...
            <div className="flex justify-center w-full px-2">
                <div className="bg-white p-2 rounded-2xl shadow-md border border-gray-200 inline-flex w-full max-w-2xl">
                    {['1mo', '3mo', '6mo'].map((c) => (
                        ...
                        >
                            {/* متن دکمه: ۱ ماهه / ۳ ماهه ... */}
                            {/* @ts-ignore */}
                            <span className="uppercase tracking-wider whitespace-nowrap">{text.cycles[c]}</span>
                            
                            {/* بج تخفیف (کنار متن) */}
                            {c === '3mo' && <span className={\`text-[10px] px-2 py-0.5 rounded-full font-extrabold \${cycle === c ? 'bg-white/20 text-white' : 'bg-green-100 text-green-700'}\`}>-5%</span>}
                            {c === '6mo' && <span className={\`text-[10px] px-2 py-0.5 rounded-full font-extrabold \${cycle === c ? 'bg-white/20 text-white' : 'bg-red-100 text-red-600 animate-pulse'}\`}>-10%</span>}
...
console.log("✅ متن دکمه‌های اشتراک (۱ ماهه، ۳ ماهه، ...) اضافه شد و ظاهرشان اصلاح گردید.");