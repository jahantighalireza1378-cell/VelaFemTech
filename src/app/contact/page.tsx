...
import React, { useState, useEffect } from 'react'; // ✅ افزودن React در اینجا حیاتی است
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            ...
                            <input required type="text" value={formData.name} className="w-full p-4 rounded-xl border bg-[#F9F7F2] focus:border-[#D4AF37] outline-none" onChange={e => setFormData({...formData, name: e.target.value})} />
                        </div>
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-[#1A2A3A]">{t.form.email}</label>
                            <input required type="email" value={formData.email} className="w-full p-4 rounded-xl border bg-[#F9F7F2] focus:border-[#D4AF37] outline-none" onChange={e => setFormData({...formData, email: e.target.value})} />
                        </div>
                    </div>
                    <div className="space-y-2">
                        <label className="text-sm font-bold text-[#1A2A3A]">{t.form.subject}</label>
                        <input required type="text" value={formData.subject} className="w-full p-4 rounded-xl border bg-[#F9F7F2] focus:border-[#D4AF37] outline-none" onChange={e => setFormData({...formData, subject: e.target.value})} />
                    </div>
                    <div className="space-y-2">
                        <label className="text-sm font-bold text-[#1A2A3A]">{t.form.msg}</label>
                        <textarea required rows={4} value={formData.message} className="w-full p-4 rounded-xl border bg-[#F9F7F2] focus:border-[#D4AF37] outline-none" onChange={e => setFormData({...formData, message: e.target.value})}></textarea>
                    </div>
                    <button type="submit" disabled={status === 'loading'} className="w-full bg-[#1A2A3A] text-white py-4 rounded-xl font-bold hover:bg-[#D4AF37] transition flex items-center justify-center gap-2">
