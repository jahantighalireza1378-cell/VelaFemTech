...
export default function BuilderPage() {
  ...
  // تغییر نام متغیرها برای هماهنگی کامل
  const [cycle, setCycle] = useState<'1' | '3' | '6'>('3'); 
  ...
  const t: any = {
    FA: {
      ...
      // ✅ کلیدها اصلاح شدند: 1, 3, 6
      cycles: { '1': '۱ ماهه', '3': '۳ ماهه', '6': '۶ ماهه' },
      billing: { '1': 'پرداخت ماهانه', '3': 'پرداخت یکجا (۳ ماه)', '6': 'پرداخت یکجا (۶ ماه)' },
      save: 'تخفیف',
      features: {
    ...
    EN: {
      ...
      cycles: { '1': '1 Month', '3': '3 Months', '6': '6 Months' },
      billing: { '1': 'Billed monthly', '3': 'Billed every 3 months', '6': 'Billed every 6 months' },
      save: 'SAVE',
      features: {
    ...
    TR: {
      ...
      cycles: { '1': '1 Ay', '3': '3 Ay', '6': '6 Ay' },
      billing: { '1': 'Aylık ödeme', '3': '3 ayda bir ödeme', '6': '6 ayda bir ödeme' },
      save: 'İNDİRİM',
      features: {
    ...
    RU: {
      ...
      cycles: { '1': '1 Месяц', '3': '3 Месяца', '6': '6 Месяцев' },
      billing: { '1': 'Ежемесячно', '3': 'Раз в 3 мес.', '6': 'Раз в 6 мес.' },
      save: 'СКИДКА',
      features: {
  ...
  const cycleConfig = {
    '1': { months: 1, discount: 0, pointMultiplier: 1 },
    '3': { months: 3, discount: 0.05, pointMultiplier: 1.2 }, 
    '6': { months: 6, discount: 0.10, pointMultiplier: 1.5 }
  };
  ...
  const calculateSmartPrice = (data: any) => {
    ...
    // ۱. قیمت ماهانه با تخفیف
    const monthlyPrice = Math.floor(basePrice * (1 - discount));
    
    // ۲. قیمت کل (تعداد ماه * قیمت ماهانه)
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        {dates && (
          <div className="animate-fade-in-up space-y-8 mt-6">
            ...
            {/* ✅ تعمیر دکمه اکو: فاصله مناسب از پایین */}
            <div className="flex justify-center mb-10">
                 <div className="w-full max-w-md transform scale-100">
                    <EcoToggle language={lang} onToggle={setIsEco} />
                 </div>
            </div>

            {/* ✅ تعمیر دکمه‌های اشتراک: هماهنگی نام‌گذاری‌ها */}
                ...
                <div className="bg-white p-2 rounded-2xl shadow-md border border-gray-200 inline-flex w-full max-w-2xl">
                    {['1', '3', '6'].map((c) => (
                        <button
                            ...
                            className={\`flex-1 py-4 rounded-xl text-sm md:text-base font-bold transition-all duration-300 flex flex-row items-center justify-center gap-2 relative \${cycle === c ? 'bg-vela-navy text-white shadow-lg transform scale-[1.02]' : 'text-gray-400 hover:bg-gray-50 hover:text-vela-navy'}\`}
                        >
                            {/* متن دکمه الان باید درست نمایش داده شود */}
                            {/* @ts-ignore */}
                            <span className="uppercase tracking-wider whitespace-nowrap">{text.cycles[c]}</span>
                            
                            {/* بج تخفیف */}
                            {c === '3' && <span className={\`text-[10px] px-2 py-0.5 rounded-full font-extrabold \${cycle === c ? 'bg-white/20 text-white' : 'bg-green-100 text-green-700'}\`}>-5%</span>}
                            {c === '6' && <span className={\`text-[10px] px-2 py-0.5 rounded-full font-extrabold \${cycle === c ? 'bg-white/20 text-white' : 'bg-red-100 text-red-600 animate-pulse'}\`}>-10%</span>}
...
console.log("✅ تعمیرات نهایی: متن دکمه‌ها اصلاح شد + فاصله دکمه اکو درست شد.");